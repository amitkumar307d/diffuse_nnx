...
def get_config(options='imagenet_64-B_2'):
    ...
    encoder_preset = ml_collections.ConfigDict(common_specs._imagenet_rae_encoder_presets[data_options])

    network_options, patch_size = network_options.split('_')
    network_preset = ml_collections.ConfigDict(common_specs._ddt_network_presets[network_options])

    # General
    config = ml_collections.ConfigDict()

    config.trainer             = 'DiT_ImageNet'
    config.exp_name            = f'RAE_{options}'
    config.project_name        = 'RAE'
    config.seed                = 0
    ...
    config.network_class       = 'lightning_ddt'
    ...
    # size
    config.network.num_encoder_blocks  = network_preset.num_encoder_blocks
    config.network.num_decoder_blocks  = network_preset.num_decoder_blocks

    config.network.encoder_hidden_size = network_preset.encoder_hidden_size
    config.network.encoder_num_heads   = network_preset.encoder_num_heads

    config.network.decoder_hidden_size = network_preset.decoder_hidden_size
    config.network.decoder_num_heads   = network_preset.decoder_num_heads
    config.network.mlp_ratio           = 4.0

    # time embed
    config.network.continuous_time_embed = True
    config.network.freq_embed_size       = 512
    ...
    # lightning
    config.network.qk_norm     = False
    config.network.swiglu      = True
    config.network.use_rope    = True
    config.network.rms_norm    = True
    config.network.adaln_shift = True

    config.network.mlp_dropout    = 0.0
    config.network.attn_w_dropout = 0.0
    config.network.attn_o_dropout = 0.0
    ...
    config.sampler_class              = 'euler'
    config.sampler.num_sampling_steps = 50
    config.sampler.sampling_time_dist = 'uniform'
    config.sampler.sampling_time_kwargs = ml_collections.ConfigDict()
    config.sampler.sampling_time_kwargs.t_start = 1.0
    config.sampler.sampling_time_kwargs.t_end   = 0.0
    config.sampler.sampling_time_kwargs.t_shift_base = config.interface.t_shift_base
    config.sampler.sampling_time_kwargs.t_shift_cur  = 16 * 16 * 768

    # EMA
    config.ema = ml_collections.ConfigDict()
    config.ema_class = "ema"
    config.ema.decay = 0.9995
