"""File containing the training loop for DiT on ImageNet. (Improved Version with Mixed Precision)"""
...
def train_step(
    ...
    encoder: Any,  # StabilityVAE passed as static
    key: jax.random.PRNGKey,
    use_bf16: bool, # Static flag for mixed precision
):
    """Training step for DiT on ImageNet. **All updates happened in-place.**
    
    Args:
    - state: state of the model & optimizer (Master weights in FP32).
    - ema_state: ema for maintaining exp moving average.
    - batch: input data.
    - graph: graph of the nnx model.
    - ema_graph: graph of the ema model.
    - encoder: VAE encoder object (static).
    - key: PRNGKey for the encoder.
    - use_bf16: whether to use bfloat16 for compute.
    """

    if use_bf16:
        # Cast master weights to BF16 for compute, only for floating point arrays!
        state_compute = jax.tree.map(lambda x: x.astype(jnp.bfloat16) if isinstance(x, jax.Array) and jnp.issubdtype(x.dtype, jnp.floating) else x, state)
    else:
        state_compute = state

    optimizer = nnx.merge(graph, state_compute)
    ...
    if use_bf16:
        # Cast gradients back to FP32 for updating master weights, only for floating point arrays!
        grads = jax.tree.map(lambda x: x.astype(jnp.float32) if isinstance(x, jax.Array) and jnp.issubdtype(x.dtype, jnp.floating) else x, grads)
    ...
    # Split the updated FP32 state to return
    _, state = nnx.split(optimizer_fp32)
...
def train_and_evaluate(
    ...
    if config.visualize.get('on'):
        ...
        if config.visualize.get('visualize_reconstruction') and not config.data.get('latent_dataset', False):
    ...
    encode_key = jax.random.fold_in(
        jax.random.PRNGKey(config.seed), jax.process_index()
    )

    # Check if we should use mixed precision
    use_bf16 = (config.get('dtype') == 'bfloat16')
    ...
    p_train_step = jax.jit(
        ...
        static_argnums=(3, 4, 5, 7), # Added 7 for use_bf16
        donate_argnums=(0, 1),
    )
    ...
    for i in range(step, config.total_steps):
        # --- START PROFILER BLOCK ---
        if i == 1000:
            logging.info("Starting XProf trace at step 1000...")
            jax.profiler.start_trace(workdir)
            
        if i == 1020:
            logging.info("Stopping XProf trace at step 1020...")
            jax.profiler.stop_trace()
        # --- END PROFILER BLOCK ---

        encode_key, step_key = jax.random.split(encode_key)
        batch = data_utils.parse_batch(
            next(loader_iter), encoder, mesh, key=step_key, detector=detector
        )
        ...
        with jax.profiler.StepTraceAnnotation('train', step_num=step):
            ...
            if config.get('log_every_steps'):
                if (step + 1) % config.log_every_steps == 0:
                    ...
                    logging.info(
                        f"Step {step}: "
                        + ", ".join([f"{k}: {v:.4f}" for k, v in summary.items() if k != 'step'])
                    )
                    metrics_history = defaultdict(list)
                    train_metrics_last_t = time.time()
